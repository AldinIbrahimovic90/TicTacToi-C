...
int main()
{
    ...
    char x = 'X', o = 'O', p = ' ';
    char con[30], winer, symbol, first_player[30], *pfirst_player, table[3][3] = {{p,p,p},{p,p,p},{p,p,p}}, *ptable, new_game[30], now_play[30], *pnow_play, no[] = "no";
    int value, game, row, column, i, draw = 0, number = 0;
    ...
    {
        number = fscanf( dr, "%s %d %s %d %d", Player1.name, &Player1.wins, Player2.name, &Player2.wins , &draw);
    }
    fclose(dr);
    ...
    while(game == 1){
        ...
        for(i = 0; i<9; i++){
            while(1){
                printf("%s, enter the row and column where you want to put %c ? ( 1 1 )\n\n", pnow_play, symbol);
        ...
        scanf("%s", &new_game);
        if(!strcmp(new_game,no)){
            //write in file
            di = fopen("old.txt", "w");
            fprintf(di, "%s\n%d\n%s\n%d\n%d\n", Player1.name, Player1.wins, Player2.name, Player2.wins, draw);
            fclose(di);
            game = 0;
        }
    }
